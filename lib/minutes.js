...
// 요구사항 3-1: 재적 22명 = 회장단 3명(회장/총무/재무) + 기타임원 5명(윤리위원장/클럽트레이너/
// 클럽코디네이터/주무사찰/상조회장) + 이사 7명 + 분과위원장 7명. 회의록 "출석 및 성원 상세 현황"을
// 이 네 그룹으로 나눠 보여준다.
const EXECUTIVE_POSITIONS = new Set(["회장", "총무", "재무"]);
const OTHER_OFFICER_POSITIONS = new Set(["윤리위원장", "클럽트레이너", "클럽코디네이터", "주무사찰", "상조회장"]);
export const ATTENDANCE_GROUP_ORDER = ["회장단", "기타임원", "이사", "분과위원장"];

export function categoryOf(position) {
  if (EXECUTIVE_POSITIONS.has(position)) return "회장단";
  if (OTHER_OFFICER_POSITIONS.has(position)) return "기타임원";
...
// "1) ...\n2) ..." 형태면 항목별로 잘라 배열로 돌려준다(줄바꿈 포함 여러 줄짜리 항목도 하나로 묶음).
// 번호가 전혀 없으면 전체를 항목 1개짜리로 취급 — 호출부에서 번호 리스트 대신 문단으로 보여준다.
export function parseResolutionItems(resolutionText) {
  if (!resolutionText) return { items: [], numbered: false };
  // Windows(\r\n)로 입력된 텍스트는 각 줄 끝에 \r이 남는데, 정규식의 $가 그 \r 앞에서
  // 걸리지 않고 매치가 실패한다(줄 끝까지 못 감) — 그래서 줄바꿈을 \n으로 통일해두고 시작한다.
  const lines = resolutionText.replace(/\r\n?/g, "\n").split("\n");
  const itemStart = /^(\d+)\)\s*(.*)$/;
  const numbered = lines.some((l) => itemStart.test(l));
  if (!numbered) return { items: [resolutionText], numbered: false };

  // 번호를 못 알아본 줄(예: 따옴표로 시작하는 줄)이 첫 번호 항목보다 앞에 있어도 버리지 않고
  // 그 앞부분을 하나의 항목으로 살려서 보여준다 — 형식이 어긋나도 내용이 조용히 사라지지 않게.
  const items = [];
  let current = [];
