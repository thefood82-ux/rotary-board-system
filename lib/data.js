...
// 홈 화면 게시판에서 "회의록 보기" 링크를 회의록이 있는 항목에만 보여주기 위한 존재 여부 배치 조회.
export async function getMeetingIdsWithMinutes(meetingIds) {
  if (!meetingIds || meetingIds.length === 0) return new Set();
  const rows = await pgSelect("meeting_minutes", {
    select: "meeting_id",
