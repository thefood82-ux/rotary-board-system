...
> 참고: 이메일 인증(확인 링크) 절차는 생략함. 관리자의 명부 매칭 승인이 실질적인 가입 검증 절차이므로, 이메일 소유 확인은 필수 요건에서 제외 (2026-08-29 확정).
...
> **양식 확정 (2026-08-29)**: 실제 클럽 회의록 형식을 기준으로 화면을 구조화한다 —
> 작성자 / 회의명 / 일시 / 성원보고 / 출석 및 성원 상세 현황(회장단·이사·분과위원장 그룹별)
> / 심의 및 의결 사항의 5개 항목.
> - **작성자**: 로그인한 관리자 본인의 명부 직책+이름을 기본값으로 채우되 직접 수정 가능.
> - **회의명**: "새송탄로타리클럽 {YYYY-YY}년도 제N차 이사회" 자동 생성 — N은 `meetings`에
>   별도 컬럼을 두지 않고 그 회기 내 `meeting_date` 오름차순 위치로 화면에서 계산한다.
> - **일시**: `meetings.meeting_date`에서 항상 새로 읽어와 표시 — 수정 불가(값을 바꾸려면
>   회의 자체를 수정해야 함).
> - **성원보고 / 출석 및 성원 상세 현황**: `attendance_responses` 기준 자동 계산해 채우되
>   텍스트로 자유롭게 수정 가능 (자동 채움은 회의록이 아직 없을 때, 즉 최초 1회만 실행).
> - **심의 및 의결 사항**: 자유 입력, 자동 채움 없음.
> - 저장은 "확정" 버튼을 누르기 전까지 타이핑을 멈추면 자동저장(디바운스)되며, 확정 시
>   `meeting_minutes.status`가 `final`로 바뀌고 편집이 잠긴다(초안으로 되돌리기 가능).
> - `meetings.meeting_date`는 이 결정과 함께 `date`에서 `timestamptz`로 확장해
>   날짜와 시간을 함께 저장한다 (등록 화면에 시간 입력란 추가).
