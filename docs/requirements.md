...
- 작성일: 2026-08-28
- 상태: **초안 — 아래 "결정 필요 사항"에 대한 답변 후 확정**
...
## 7. 결정 필요 사항 (구현 전 확정 필요)

① **회기 교체 방식**: 회기가 바뀌면 `board_members`에 새 회기용 행을 새로 추가(이력 보존)하는 방식으로 할까요, 아니면 기존 행을 덮어쓰는 방식으로 할까요? (회계 시스템의 `pledges`처럼 회기마다 새 행을 만드는 방식을 기본안으로 제안합니다.)

② **가입 시 본인 매칭 방식**: 가입자가 명부에서 본인 이름을 **드롭다운으로 선택**하게 할까요, 아니면 이름을 **텍스트로 입력**한 뒤 관리자가 매칭하게 할까요? (드롭다운 선택 + 관리자 최종 승인을 기본안으로 제안합니다.)

③ **위임(대리) 규칙**: 한 이사가 여러 명의 위임을 동시에 받을 수 있나요, 아니면 1인당 1건까지만 위임을 받을 수 있나요? (로타리 정관/세칙 규정에 따라 다를 수 있어 확인이 필요합니다.)

④ **회의 마감 처리**: 관리자가 회의를 "마감"으로 전환하면 이후 이사들의 응답 수정을 잠글까요? 마감 기한(예: 회의 전날 자정)을 자동으로 둘까요, 관리자가 수동으로 마감할까요?

⑤ **회의록 연동 범위**: 시스템 안에 회의록 작성 화면을 따로 만들까요, 아니면 성원현황을 텍스트(워드/한글에 붙여넣기용)로 생성하는 기능만 만들까요? (MVP 범위 확인 필요)

⑥ **소집 알림**: 관리자가 소집을 등록하면 22명에게 이메일 등으로 알림을 자동 발송할까요? (MVP 범위에 포함할지 여부)

⑦ **관리자 권한 범위**: 총무(임상재) 외에 회장도 admin 권한을 가지나요?
